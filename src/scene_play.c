...
scene_play_T* init_scene_play()
{
    ...
    s_play->text_computer_score = init_actor_text((WINDOW_WIDTH / 2) + 120.0f, 64.0f, 0.0f, "0", 255, 255, 255);
    s_play->text_player_score_buffer = calloc(16, sizeof(char));
    s_play->text_computer_score_buffer = calloc(16, sizeof(char));

    dynamic_list_append(s->actors, init_actor_pad(16.0f, (WINDOW_HEIGHT / 2) - 32, 0.0f, 1));
    dynamic_list_append(s->actors, init_actor_pad(WINDOW_WIDTH - 32.0f, (WINDOW_HEIGHT / 2) - 32, 0.0f, 0));
    dynamic_list_append(s->actors, init_actor_ball(WINDOW_WIDTH / 2, (WINDOW_HEIGHT / 2) - 16, 0.0f));
...
void scene_play_draw(scene_T* self)
{
    ...
    render_2D_positioned_2D_mesh(WINDOW_WIDTH / 2, 0.0f, 16.0f, WINDOW_HEIGHT, 255.0f, 255.0f, 255.0f, self->VAO, self->camera->projection_view);
